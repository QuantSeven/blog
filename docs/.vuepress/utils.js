...
function getDirFiles(dirname) {
    ...
    let ignores = [".git", ".gitignore", "node_modules", "yarn.lock", "package.json", "package-lock.json"];
...
function getRootDir(rootPath) {
    ...
    files.forEach(function (file, index, array) {
        ...
        // 判断是否为文件夹
        if (stat.isDirectory()) {
            //let navObj = {text: file, ariaLabel: file};
            let navObj = {text: file, link: "/" + file  + "/"};
            //items 循环
            /* let childrenFiles = getDirFiles(realpath);
            if(childrenFiles.length > 0) {
                navObj['items'] = []
                childrenFiles.forEach(function (childrenFile, childrenIndex, childrenArray) {
                    let childrenRealpath = path.join(realpath, childrenFile);
                    let childrenStat = fs.lstatSync(childrenRealpath);
                    if (childrenStat.isDirectory()) {
                        navObj['items'].push({text: childrenFile, link: "/" + file  + "/" + childrenFile + "/"})
                    }
                })
            } */
            nav.push(navObj);
